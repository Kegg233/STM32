...
void OUTPUT_Init(void)
{
	...
	while(1){
		...
		if(change++ == 0xee)
		{
			...
			if(VCC_BASE > Mode_value){
				Mode = 1;
				if(Mode != old_Mode){
					printf("change  this mode is %d \r\n",Mode);
					printf("外部电压：%f V\r\n",Mode_value);
					MODE(1);
					old_Mode = 1;
				}
		...
		if(Mode == 1){
			// PID 算法 P ，计算出下一个时刻的占空比
			vcc_zkb_base = vcc_zkb_base+(  VCC_BASE - ADC_V )*0.5-0.001;
			// 采样值与基准值对比，防止超过100或小于0
			if(vcc_zkb_base > 75){
				vcc_zkb_base = 75;
			}else if(vcc_zkb_base < 10 ){
				vcc_zkb_base = 10;
			}
			// 如果电流保护使能的话， 启动过流关闭系统
			if(I_PROTECT){
