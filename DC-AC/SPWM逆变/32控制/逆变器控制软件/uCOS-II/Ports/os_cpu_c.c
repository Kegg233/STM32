...
void  OS_CPU_SysTickInit (void)
{
	...
	RCC_GetClocksFreq(&rcc_clocks);			 					      //获取 HCLK时钟

    cnts = ((INT32U)rcc_clocks.HCLK_Frequency) / OS_TICKS_PER_SEC;	  //hclk 除 任务切换次数 
																	  
    OS_CPU_CM3_NVIC_ST_RELOAD = (cnts - 1);							  //重新赋值系统滴答寄存器
                                                 /* Enable timer.                                      */
